...
public class WheelView extends View {
	...
	/** Items text color 原来是static final */
	private int ITEMS_TEXT_COLOR = 0xFF000000;

	/** Top and bottom shadows colors 原来是static final */
...
//			itemsPaint.setTextSize(TypedValue.COMPLEX_UNIT_PX,22);
		}

		if (valuePaint == null) {
			valuePaint = new TextPaint(Paint.ANTI_ALIAS_FLAG | Paint.DITHER_FLAG);
			//valuePaint.density = getResources().getDisplayMetrics().density;
			valuePaint.setTextSize(TEXT_VALUE_SIZE);//TEXT_SIZE
			valuePaint.setShadowLayer(0.1f, 0, 0.1f, 0xFFC0C0C0);
		}

		if (centerDrawable == null) {
			if(drawableVal!=0){
				centerDrawable = getContext().getResources().getDrawable(drawableVal);//R.drawable.wheel_val
			}else{
		...
		if(drawableBg!=0){
			setBackgroundResource(drawableBg);
		}else{
...
//		}
	...
	public void setTEXT_SIZE(int text_size) {
		...
		try {
			...
			//Log.e(TAG, "屏幕密度 --->" + density);
		} catch (Exception e) {
			e.printStackTrace();
		}
		
		//Log.e(TAG, "TEXT_SIZE之前 --->" + TEXT_SIZE);
		TEXT_SIZE = (int)(text_size * density);
		TEXT_VALUE_SIZE = TEXT_SIZE+2;
		//Log.e(TAG, "TEXT_SIZE之后 --->" + TEXT_SIZE);
	}
	
	public void setITEMS_TEXT_COLOR(int itemTextColor) {
		this.ITEMS_TEXT_COLOR = itemTextColor;
	}
	
	public void setSHADOWS_COLORS(int[] shadowsColors){
