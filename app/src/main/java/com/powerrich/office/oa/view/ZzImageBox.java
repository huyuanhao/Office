    ...
    private static class MyAdapter extends RecyclerView.Adapter<ViewHolder> {
        ...
        public void onBindViewHolder(final ViewHolder holder, final int position) {
            ...
            if (holder.getAdapterPosition() == getItemCount() - 1 && !lastOne) {
                ...
                holder.ivPic.setImageResource(addPic == -1 ? R.drawable.icon_boxmore_add : addPic);
                ...
                if (url != null && url.length() != 0) {
                    if (url.startsWith("http")) {
                        if (imageLoader != null) {
                            ...
                            holder.ivPic.setImageResource(defaultPic == -1 ? R.mipmap.ic_launcher : defaultPic);
                        }
                    } else {
                        if (imageLoader != null) {
                            imageLoader.onLoadImage(holder.ivPic, url);
...
//                        holder.ivPic.setImageURI(Uri.fromFile(new File(url)));//此方法极耗内存，不能用
                    }
                } else {
                    holder.ivPic.setImageResource(defaultPic == -1 ? R.mipmap.ic_launcher : defaultPic);
                }
