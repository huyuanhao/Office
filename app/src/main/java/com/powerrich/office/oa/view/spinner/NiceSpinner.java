package com.powerrich.office.oa.view.spinner;
...
public class NiceSpinner extends AppCompatTextView {
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (onItemSelectedListener != null) {
                    onItemSelectedListener.onItemSelected(parent, view, position-1, id);
                }
                adapter.notifyItemSelected(position-1);
                setText(adapter.getItemInDataset(position-1).toString());
                dismissDropDown();
            }
        });
        View view = new View(context);
        view.setBackgroundColor(context.getResources().getColor(R.color.ddd));
    ...
    public <T> void attachDataSource(@NonNull List<T> dataset, String title) {
        adapter = new NiceSpinnerAdapter<>(getContext(), dataset, textColor, backgroundSelector);
        setAdapterInternal(adapter,title);
    }

    public void setAdapter(@NonNull ListAdapter adapter, String title) {
        this.adapter = new NiceSpinnerAdapterWrapper(getContext(), adapter, textColor, backgroundSelector);
        setAdapterInternal(this.adapter,title);
    }

    private void setAdapterInternal(@NonNull NiceSpinnerBaseAdapter adapter, String title) {
        // If the adapter needs to be settled again, ensure to reset the selected index as well
        selectedIndex = 0;
        listView.setAdapter(adapter);
        setText(title);
    }
    private OnStatus onDissms=null;

    public void setOnStatus(NiceSpinner.OnStatus onDissms) {
...
//        popupWindow.setWidth(MeasureSpec.getSize(widthMeasureSpec));
        popupWindow.setHeight(dip2px(getContext(),(45*4)+24));
    ...
    public void showDropDown() {
        ...
        popupWindow.showAsDropDown((View)this.getParent());
    }
